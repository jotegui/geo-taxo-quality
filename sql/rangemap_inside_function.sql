...
DROP FUNCTION IF EXISTS public.rangemap_inside(text, geometry); -- Main function: scientific name and point geometry
DROP FUNCTION IF EXISTS public.rangemap_inside(geometry, text); -- First overload: point geometry and scientific name
DROP FUNCTION IF EXISTS public.rangemap_inside(text, double precision, double precision); -- second overload: scientific name and coordinates
DROP FUNCTION IF EXISTS public.rangemap_inside(double precision, double precision, text); -- third overload: coordinates and scientific name
...
BEGIN
    ...
    select into inside ins from (select rangemap_inside(p_sciname, p_geom) as ins) as foo;
...
BEGIN
    ...
    select into inside ins from (select rangemap_inside(p_sciname, lat, lon) as ins) as foo;
